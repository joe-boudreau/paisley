...
export class PrincipalContract  extends ContractBase {
    ...
    public async getEmployee(ctx: Context, id: string): Promise<string> {
        const existing = await ctx.stub.getState(this._getEmployeeKey(ctx, id))
        return new Employee(existing).toJson()
    }

    @Transaction()
    public async deleteEmployee(ctx: Context, id: string): Promise<string> {
        await ctx.stub.deleteState(this._getEmployeeKey(ctx, id))
        return `Successfully deleted employee ID: ${id}`
    }

    @Transaction()
    public async getAllEmployees(ctx: Context): Promise<string[]> {
        const empIterator = await ctx.stub.getStateByPartialCompositeKey(this.getName(), [])
